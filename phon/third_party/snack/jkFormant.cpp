...
double get_stat_max(POLE **pole, int nframes)
{
	int i;
	double amax, t;
...
Sound *dpform(Sound *ps, int nform, double nom_f1)
{
	...
	if (ps)
	{
		...
		fr = (double **) calloc(sizeof(double *), nform * 2);
		ba = fr + nform;
		for (i = 0; i < nform * 2; i++)
		{
			fr[i] = (double *) calloc(sizeof(double), ps->length);
		}
		...
		if (1)
		{
			...
			pcan = (short **) calloc(sizeof(short *), MAXCAN);
			for (i = 0; i < MAXCAN; i++) pcan[i] = (short *) calloc(sizeof(short), nform);
			...
			fl = (FORM **) calloc(sizeof(FORM *), ps->length);
			for (i = 0; i < ps->length; i++)
				fl[i] = (FORM *) calloc(sizeof(FORM), 1);
			...
			{    /* for all analysis frames... */
				...
				{    /* if there ARE pole frequencies available... */
					...
					fl[i]->prept = (short *) calloc(sizeof(short), ncan);
					fl[i]->cumerr = (double *) calloc(sizeof(double), ncan);
					fl[i]->cand = (short **) calloc(sizeof(short *), ncan);
					for (j = 0; j < ncan; j++)
					{    /* allocate cand. slots and install candidates */
						fl[i]->cand[j] = (short *) calloc(sizeof(short), nform);
						for (k = 0; k < nform; k++)
...
/*************************************************************************/
double integerize(double time, double freq)
{
...
Sound *lpc_poles(Sound *sp, double wdur, double frame_int, int lpc_ord, double preemp, int lpc_type, int w_type)
{
	...
	if (nfrm >= 1/*lp->buff_size >= 1*/)
	{
		...
		pole = (POLE **) calloc(nfrm/*lp->buff_size*/, sizeof(POLE *));
		datap = dporg = (short *) calloc(sizeof(short), sp->length);
		...
		{
			pole[j] = (POLE *) calloc(sizeof(POLE), 1);
			pole[j]->freq = frp = (double *) calloc(sizeof(double), lpc_ord);
			pole[j]->band = bap = (double *) calloc(sizeof(double), lpc_ord);
...
int lpcbsa(int np, double lpc_stabl, int wind, short *data, double *lpc, double *rho, double *nul1, double *nul2,
           double *energy, double preemp)
{
...
void do_fir(short *buf, int in_samps, short *bufo, int ncoef, short ic[], int invert)
{
...
int get_abs_maximum(short *d, int n)
{
	int i;
	short amax, t;
...
int dwnsamp(short *buf, int in_samps, short **buf2, int *out_samps, int insert, int decimate, int ncoef, short ic[],
            int *smin, int *smax)
{
	short *bufp, *bufp2;
	short *buft;
	...
	if (!(*buf2 = buft = (short *) calloc(sizeof(short), insert * in_samps)))
	{
		perror("calloc() in dwnsamp()");
		return (FALSE);
	}
...
Sound *Fdownsample(Sound *s, double freq2, int start, int end)
{
	...
	if ((bufout = (short **) calloc(sizeof(short *), 1)))
	{
		bufin = (short *) calloc(sizeof(short), (end - start + 1));
		for (i = start; i <= end; i++)
		{
...
#define LCSIZ 101
	...
	datain = (short *) calloc(sizeof(short), s->length);
	dataout = (short *) calloc(sizeof(short), s->length);
	...
	if (!len)
	{        /* need to create a Hanning FIR? */
		lcf = (short *) calloc(sizeof(short), LCSIZ);
		len = 1 + (LCSIZ / 2);
		fn = PI * 2.0 / (LCSIZ - 1);
...
int
formantCmd(Sound *s, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
