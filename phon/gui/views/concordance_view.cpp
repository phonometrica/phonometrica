...
#define ICN(x) wxBITMAP_PNG_FROM_DATA(x)
	...
	m_save_tool = m_toolbar->AddTool(wxID_ANY, _("Save concordance\tctrl+s"), save_icon, _("Save concordance (" CTRL_KEY "S)"));
	auto csv_tool = m_toolbar->AddTool(wxID_ANY, _("Export concordance to CSV"), ICN(export_csv), _("Export concordance to CSV"));
	m_toolbar->EnableTool(m_save_tool->GetId(), false);
	m_toolbar->AddSeparator();

	auto play_icon = ICN(play);
	m_play_tool = m_toolbar->AddTool(wxID_ANY, _("Play selection"), play_icon, _("Play selection"));

	auto stop_tool = m_toolbar->AddTool(wxID_ANY, _("Stop playing"), ICN(stop), _("Stop playing"));
	auto praat_tool = m_toolbar->AddTool(wxID_ANY, _("Open selection in Praat"), ICN(praat), _("Open selection in Praat"));
	m_toolbar->AddSeparator();

	auto view_tool = m_toolbar->AddTool(wxID_ANY, _("Open match in annotation"), ICN(eye), _("Open match in annotation"));

	m_col_tool = m_toolbar->AddTool(wxID_ANY, _("Show/hide columns"), ICN(select_column), _("Show/hide columns"), wxITEM_DROPDOWN);
...
//	ctx_tool->Check(true);
	...
	auto bookmark_tool = m_toolbar->AddTool(wxID_ANY, _("Bookmark match"), ICN(favorite24), _("Bookmark match"));
	...
	m_toolbar->Bind(wxEVT_COMMAND_TOOL_CLICKED, &ConcordanceView::OnPlaySelection, this, m_play_tool->GetId());
	m_toolbar->Bind(wxEVT_COMMAND_TOOL_CLICKED, &ConcordanceView::OnStopPlaying, this, stop_tool->GetId());
	m_toolbar->Bind(wxEVT_COMMAND_TOOL_CLICKED, &ConcordanceView::OnOpenInPraat, this, praat_tool->GetId());
	m_toolbar->Bind(wxEVT_COMMAND_TOOL_CLICKED, &ConcordanceView::OnExportToCsv, this, csv_tool->GetId());
	m_toolbar->Bind(wxEVT_COMMAND_TOOL_CLICKED, &ConcordanceView::OnViewMatch, this, view_tool->GetId());
//	m_toolbar->Bind(wxEVT_TOOL_DROPDOWN, &ConcordanceView::OnColumnButtonClicked, this, m_col_tool->GetId());
	m_toolbar->Bind(wxEVT_COMMAND_TOOL_CLICKED, &ConcordanceView::OnBookmarkMatch, this, bookmark_tool->GetId());
